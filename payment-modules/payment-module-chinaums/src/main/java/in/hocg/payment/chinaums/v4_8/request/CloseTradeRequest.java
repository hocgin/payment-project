...
public class CloseTradeRequest extends ChinaUmsPayRequest<CloseTradeResponse> {
    /**
    ...
    private String msgType = "close";

    @Override
    protected CloseTradeResponse request() {
        if (Objects.isNull(merOrderId)) {
            setMerOrderId(getMerOrderId(orderSn));
        }
        return request(CloseTradeResponse.class);
    }
}
